...
function onMessage(req, res) {
    ...
    xml2js.parseString(req.body, {trim: true}, function (error, _message) {
        if (error != null) {
            logger.error({info: "parse req body due to error", file: __filename, body: req.body, err: JSON.stringify(error)});
            return;
        }
        ...
        try {
            ...
            logger.error({ info: "process message due to error", file: __filename, message: _message, err: JSON.stringify(error)});
        }
    });
}